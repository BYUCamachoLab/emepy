...
def bragg_lumapi(args):
    ...
    for wavelength in np.linspace(wl_lower, wl_upper, num_wavelengths):
        ...
        mode_solver1 = modesolver(
            wavelength, width1, thickness, mesh=mesh, num_modes=num_modes, mode=eme.mode, eme_modes=True
        )  # First half of bragg grating

        mode_solver2 = modesolver(
            wavelength, width2, thickness, mesh=mesh, num_modes=num_modes, mode=eme.mode, eme_modes=True
