...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default="0.22",
        help="Thickness of the core (microns) (default: 0.22)",
    )

    args = parser.parse_args()

    t_ann = bragg_ann(args)
    t_lumerical_eme = lumerical_eme(
        args
    )  # Retry tomorrow with proper licensing issues gone and then make sure to fix the normalization of modes and mode overlap formulation for the other solvers
    ...
    plt.figure()
    plt.plot(
        np.linspace(1.5, 1.6, args.num_wavelengths), t_ann, label="Neural Networks"
    )
    plt.plot(
        np.linspace(1.5, 1.6, args.num_wavelengths),
        t_lumerical,
        label="Lumerical MODE FDE",
    )
