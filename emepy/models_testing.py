...
class Layer(object):
    ...
    def activate_layer(self, sources=[], start=0.0, num_periods=1, period_length=0.0):
        ...
        custom_sources = {i:ModelTools.get_sources(sources, start+i*period_length, start+self.length+i*period_length) for i in range(num_periods)}

        # If no custom sources
        if not len([i for i in custom_sources.values() if len(i)]):
            self.activated_layers = {**{0:[ActivatedLayer(modes, self.wavelength, self.length)]},**{i:[] for i in range(1,num_periods)}}

        # Other sources
        else:
            self.activated_layers = {i:ModelTools.get_source_system(modes, self.wavelength, self.length, custom_sources[i], start+i*period_length) for i in range(num_periods)}
