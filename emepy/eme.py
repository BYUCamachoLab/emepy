...
class EME(object):
                ...
                for per, layers in solved.items():
                    if not sum(
                        ...
                        self.activated_layers[per] += (
                            layers if not layers[0] is None else solved[0]
                        )
    ...
    def field_propagate(self, left_coeffs: list, right_coeffs: list) -> None:
        ...
        for m in self.custom_monitors + self.monitors:
            ...
            for per, activated_layers in tqdm(
                ...
                activated_layers = (
                    activated_layers
                    if not activated_layers is None
    ...
    def propagate(
        ...
        if left_coeffs is None:
            if not len(right_coeffs) and not len(
                [i for i in self.get_sources().values() if len(i)]
            ):
                left_coeffs = [1]
            else:
                left_coeffs = []
    ...
    def _layer_field_propagate(
        ...
        activated_layers = (
            self.activated_layers[per]
            if not self.activated_layers[per] is None
            else self.activated_layers[0]
        )
        ...
        for p in range(per):
            network = (
                self.networks[p] if not self.networks[p] is None else self.networks[0]
            )
            interface = (
                self.periodic_interfaces[p]
                if not self.periodic_interfaces[p] is None
                else self.periodic_interfaces[0]
            )
        ...
        S = _prop_all(
            *[
                t
                for t in prop
                if not (t is None) and not (isinstance(t, list) and not len(t))
            ]
        )
