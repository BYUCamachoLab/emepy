...
def rectangle_difference(subpixel: bool, dp: float):
    ...
    rect_i = rectangle_to_n(
        center=center, width=width, thickness=thickness, x=x, y=y, subpixel=subpixel, core_index=3.4, cladding_index=1.4
    )
...
def circle_difference(subpixel: bool, dp: float):
    ...
    # Initial rect
    rect_i = circle_to_n(center=center, radius=radius, x=x, y=y, subpixel=subpixel, core_index=3.4, cladding_index=1.4)

    # Final rect
    rect_f = circle_to_n(
        center=center, radius=radius + dp, x=x, y=y, subpixel=subpixel, core_index=3.4, cladding_index=1.4
    )
    return rect_f - rect_i
