...
# ModeSolvers
b_mode_solver1 = modesolver(
    b_wavelength, b_width1, b_thickness, mesh=b_mesh, num_modes=b_num_modes
)  # First half of bragg grating
b_mode_solver2 = modesolver(
    b_wavelength, b_width2, b_thickness, mesh=b_mesh, num_modes=b_num_modes
...
t_mode_solver_input = modesolver(
    t_wavelength, t_input_width, t_thickness, mesh=t_mesh, num_modes=t_num_modes
)  # Input waveguide
t_mode_solver_output = modesolver(
    t_wavelength, t_output_width, t_thickness, mesh=t_mesh, num_modes=t_num_modes
)  # Output waveguide

# Taper ModeSolvers
widths = np.linspace(t_input_width, t_output_width, t_num_layers)  # Widths of each layer
t_mode_solvers_taper = [
    modesolver(t_wavelength, width, t_thickness, mesh=t_mesh, num_modes=t_num_modes) for width in widths
]  # Taper waveguides
...
def bragg_grating(parallel: bool, num_periods: int):
    ...
    b_eme.add_layer(
        Layer(deepcopy(b_mode_solver2), b_num_modes, b_wavelength, b_length)
    )  # Second half of bragg grating
...
def taper(parallel: bool):
    ...
    t_eme.add_layer(
        Layer(deepcopy(t_mode_solver_output), t_num_modes, t_wavelength, t_output_length)
    )  # Output waveguide
